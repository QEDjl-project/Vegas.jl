...
@kernel function vegas_binning_kernel!(bins_buffer::AbstractMatrix{T}, ndi_buffer::AbstractMatrix{Int}, values::AbstractMatrix{T}, target_weights::AbstractVector{T}, grid_lines, func::Function, @Const(Ng), ::Val{D}) where {T<:Number, D}
...
function binning_vegas!(backend, bins_buffer::AbstractMatrix{T}, ndi_buffer::AbstractMatrix{Int}, buffer::VegasBatchBuffer{T, N, D, V, W, J}, grid::VegasGrid{T, Ng, D, G}, func::Function) where {T <: AbstractFloat, N, D, V, W, J, Ng, G}
    ...
    @debug "Calling binning kernel with $(nbins) bins * $(D) dims = $(nbins * D) threads"
